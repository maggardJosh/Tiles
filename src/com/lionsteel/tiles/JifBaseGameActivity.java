 ...
 * 
 * 
 * Simply removed mRenderSurfaceView.onPause() to keep textures from being unloaded
 * Might not be very good... going to extensively test before keeping this.
 */
public abstract class JifBaseGameActivity extends BaseActivity implements IGameInterface, IRendererListener {
	// ===========================================================
	// Constants
	// ===========================================================
	...
	protected synchronized void onCreateGame() {
		...
		final OnPopulateSceneCallback onPopulateSceneCallback = new OnPopulateSceneCallback() {
			...
			public void onPopulateSceneFinished() {
				try {
					if(BuildConfig.DEBUG) {
						Debug.d(JifBaseGameActivity.this.getClass().getSimpleName() + ".onGameCreated" + " @(Thread: '" + Thread.currentThread().getName() + "')");
					}

					JifBaseGameActivity.this.onGameCreated();
				} catch(final Throwable pThrowable) {
					Debug.e(JifBaseGameActivity.this.getClass().getSimpleName() + ".onGameCreated failed." + " @(Thread: '" + Thread.currentThread().getName() + "')", pThrowable);
				}

				JifBaseGameActivity.this.callGameResumedOnUIThread();
			}
		};
		...
		final OnCreateSceneCallback onCreateSceneCallback = new OnCreateSceneCallback() {
			...
			public void onCreateSceneFinished(final Scene pScene) {
				JifBaseGameActivity.this.mEngine.setScene(pScene);

				try {
					if(BuildConfig.DEBUG) {
						Debug.d(JifBaseGameActivity.this.getClass().getSimpleName() + ".onPopulateScene" + " @(Thread: '" + Thread.currentThread().getName() + "')");
					}

					JifBaseGameActivity.this.onPopulateScene(pScene, onPopulateSceneCallback);
				} catch(final Throwable pThrowable) {
					Debug.e(JifBaseGameActivity.this.getClass().getSimpleName() + ".onPopulateScene failed." + " @(Thread: '" + Thread.currentThread().getName() + "')", pThrowable);
				}
			}
		};
		...
		final OnCreateResourcesCallback onCreateResourcesCallback = new OnCreateResourcesCallback() {
			...
			public void onCreateResourcesFinished() {
				try {
					if(BuildConfig.DEBUG) {
						Debug.d(JifBaseGameActivity.this.getClass().getSimpleName() + ".onCreateScene" + " @(Thread: '" + Thread.currentThread().getName() + "')");
					}

					JifBaseGameActivity.this.onCreateScene(onCreateSceneCallback);
				} catch(final Throwable pThrowable) {
					Debug.e(JifBaseGameActivity.this.getClass().getSimpleName() + ".onCreateScene failed." + " @(Thread: '" + Thread.currentThread().getName() + "')", pThrowable);
				}
			}
		};
...
//		this.mRenderSurfaceView.onPause();
	...
	private void callGameResumedOnUIThread() {
		JifBaseGameActivity.this.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				JifBaseGameActivity.this.onResumeGame();
			}
		});
	}
	...
	protected void onSetContentView() {
		...
		this.setContentView(this.mRenderSurfaceView, JifBaseGameActivity.createSurfaceViewLayoutParams());
	}
